...
void run_simple_server() {
  ...
  std::cout 
            << "ACTION: waiting for a client connection..." << std::endl;
  ...
  std::cout 
            << "SUCCESS: Client connected! New socket FD for communication: "
            << new_socket << std::endl;
  ...
  while (true){
    ...
    if(bytes_recieved <= 0){
      if (bytes_recieved == 0){
        std::cout 
                  << "\nSTATUS: Client disconnected gracefully." 
                  << std::endl;
      }
      else{
        std::cerr 
    ...
    if(message.find("NICK") != std::string::npos){
      ...
      std::cout 
                << "DEBUG: Found nickname: "
                << client_nick
                << std::endl;

      std::string rpl_001 = ":localhost 001 " + client_nick + " :Welcome to the IRC Network!\r\n";
      std::string rpl_002 = ":localhost 002 " + client_nick + " :Your host is localhost, running version 1.0\r\n";
      std::string rpl_003 = ":localhost 003 " + client_nick + " :This server was created Jan 2026\r\n";
      std::string rpl_004 = ":localhost 004 " + client_nick + " localhost 1.0 o o\r\n";

      send(new_socket, rpl_001.c_str(), rpl_001.length(), 0);
      send(new_socket, rpl_002.c_str(), rpl_002.length(), 0);
      send(new_socket, rpl_003.c_str(), rpl_003.length(), 0);
      send(new_socket, rpl_004.c_str(), rpl_004.length(), 0);

      std::cout 
                << "DEBUG: Sent full registration to "
                << client_nick 
                << std::endl;
    }
